...
from typing import (
    ...
    Callable, Generator, List, Optional, TextIO, Tuple,
)
...
from redis_cache_lock.utils import wrap_generate_func, sentinel_client_acm, monotime, HistoryHolder
...
class Worker:
    ...
    def _make_gen_func(
            self, gen_time: float, block_time: float,
    ) -> Callable[[], Awaitable[Tuple[bytes, dict]]]:

        async def gen_func(
                # self: Worker = self,
                gen_time: float = gen_time, block_time: float = block_time,
        ) -> dict:
    ...
    async def _arun_i(self) -> dict:
        ...
        sentinel_cli = await cm_stack.enter_async_context(sentinel_acm(
            sentinels=sentinel_cfg['sentinels'],
            db=sentinel_cfg['db'],
            password=sentinel_cfg['password'],
        ))
        self._sentinel_cli = sentinel_cli
        self._client_acm = sentinel_client_acm(sentinel_cli, sentinel_cfg['service_name'])
